...
def main():
    ...
    for stock_name, stock_code in STOCK_TICKERS.items():
        ...
        # 3. 周线量化策略
        print("\n3. 执行 weekly_quantitative_strategy.py")
        run_command(f"python weekly_quantitative_strategy.py --ticker {stock_code}")
        
        # 4. 周线策略优化
        print("\n4. 执行 weekly_strategy_optimization.py")
        run_command(f"python weekly_strategy_optimization.py --ticker {stock_code}")
        
        # 5. 周线股价预测
        print("\n5. 执行 weekly_stock_prediction.py")
        run_command(f"python weekly_stock_prediction.py --ticker {stock_code}")
        
        # 6. 周线AI分析
        print("\n6. 执行 weekly_stock_ai_local_analyzer.py")
        run_command(f"python weekly_stock_ai_local_analyzer.py --ticker {stock_code}")
