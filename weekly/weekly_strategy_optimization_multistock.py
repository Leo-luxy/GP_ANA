# weekly_strategy_optimization_multistock.py
# 功能：为多只股票优化周线量化交易策略参数，找到一组通用参数
# 实现原理：
...
from .weekly_strategy_optimization import WeeklyStrategyOptimizer

class MultiStockStrategyOptimizer:
    def __init__(self):
        self.stocks = []
        self.stock_results = {}
    ...
    def get_stock_list(self):
        ...
        for stock in stock_dirs:
            weekly_data_file = os.path.join(data_dir, stock, f"{stock}_weekly_data.csv")
            if os.path.exists(weekly_data_file):
                self.stocks.append(stock)
                print(f"  找到股票: {stock}")
            else:
                print(f"  跳过股票 {stock}: 缺少周线数据文件")
    ...
    def optimize_all_stocks(self):
        ...
        for stock in self.stocks:
            print(f"\n优化股票: {stock}")
            file_path = os.path.join(DATA_DIR, stock, f"{stock}_weekly_data.csv")
    ...
    def update_config(self, common_params):
        ...
        try:
            ...
            print("\n=== 配置文件更新完成 ===")
            print(f"已将通用参数写入config.py中的WEEKLY_STRATEGY_CONFIG:")
...
if __name__ == "__main__":
    # 运行多股票策略优化
    optimizer = MultiStockStrategyOptimizer()
