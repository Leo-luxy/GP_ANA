# weekly_strategy_optimization.py
# 功能：优化周线量化交易策略的参数，提高策略性能
# 实现原理：
# 1. 加载股票周线历史数据，包括价格和技术指标
...
from config import DATA_DIR, WEEKLY_STRATEGY_CONFIG, WEEKLY_OPTIMIZATION_CONFIG

# 设置中文字体
plt.rcParams['font.sans-serif'] = ['Arial Unicode MS']  # 用于macOS的中文字体
plt.rcParams['axes.unicode_minus'] = False  # 用来正常显示负号

class WeeklyStrategyOptimizer:
    ...
    def backtest_strategy(self):
        ...
        try:
            ...
            if len(portfolio_value) > 1:
                ...
                annual_return = (portfolio_value[-1] / self.initial_capital) ** (52 / num_trading_weeks) - 1 if num_trading_weeks > 0 else 0
                volatility = np.std(returns) * np.sqrt(52) if num_trading_weeks > 0 else 0
                risk_free_rate = 0.03
    ...
    def optimize_parameters(self):
        ...
        # 使用配置文件中的参数搜索空间
        rsi_buy_thresholds = WEEKLY_OPTIMIZATION_CONFIG['rsi_buy_thresholds']
        rsi_sell_thresholds = WEEKLY_OPTIMIZATION_CONFIG['rsi_sell_thresholds']
        bb_buy_mults = WEEKLY_OPTIMIZATION_CONFIG['bb_buy_mults']
        bb_sell_mults = WEEKLY_OPTIMIZATION_CONFIG['bb_sell_mults']
    ...
    def plot_optimization_results(self, all_results):
        ...
        plt.tight_layout()
        chart_path = os.path.join(stock_dir, f'{self.ticker}_weekly_optimization_results.png')
        plt.savefig(chart_path)
        print(f"周线优化结果图表已保存为: {chart_path}")
    ...
    def update_config(self, best_params):
        ...
        try:
            ...
            # 找到WEEKLY_STRATEGY_CONFIG的位置并更新参数
            in_weekly_config = False
            new_lines = []
            
            for line in lines:
                if 'WEEKLY_STRATEGY_CONFIG = {' in line:
                    in_weekly_config = True
                    new_lines.append(line)
                elif in_weekly_config and 'rsi_buy_threshold' in line:
                    new_lines.append(f"    'rsi_buy_threshold': {rsi_buy_threshold},  # RSI买入阈值\n")
                elif in_weekly_config and 'rsi_sell_threshold' in line:
                    new_lines.append(f"    'rsi_sell_threshold': {rsi_sell_threshold},  # RSI卖出阈值\n")
                elif in_weekly_config and 'bb_buy_mult' in line:
                    new_lines.append(f"    'bb_buy_mult': {bb_buy_mult},  # 布林带买入倍数\n")
                elif in_weekly_config and 'bb_sell_mult' in line:
                    new_lines.append(f"    'bb_sell_mult': {bb_sell_mult},  # 布林带卖出倍数\n")
                else:
                    new_lines.append(line)
                
                # 检查是否结束了WEEKLY_STRATEGY_CONFIG
                if in_weekly_config and '}' in line and not line.strip().startswith('#'):
                    in_weekly_config = False
            ...
            print("\n=== 配置文件更新完成 ===")
            print(f"已将最佳参数写入config.py中的WEEKLY_STRATEGY_CONFIG:")
...
if __name__ == "__main__":
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="优化周线量化交易策略的参数")
    ...
    # 运行策略优化
    optimizer = WeeklyStrategyOptimizer(file_path)
