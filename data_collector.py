...
if __name__ == "__main__":
    ...
    parser.add_argument('--ticker', help="股票代码，例如：600519.SH（上交所）、000001.SZ（深交所），默认处理所有股票")
    ...
    for ticker_code in ticker_list:
        ...
        for name, code in STOCK_TICKERS.items():
            if code == ticker_code:
                ticker_name = name
                break
        print(f"处理股票: {ticker_code} ({ticker_name})")
        ...
        # 确保数据目录存在
        stock_dir = os.path.join(DATA_DIR, ticker_code)
        if not os.path.exists(stock_dir):
            os.makedirs(stock_dir)
        
        # 构建文件路径
        qfq_file = os.path.join(stock_dir, f"{ticker_code}_qfq.csv")
        ...
        if os.path.exists(qfq_file):
            ...
            try:
                ...
                if not df.empty:
                    ...
                    print(f"文件存在，从 {start_date} 到 {end_date} 获取数据")
                else:
                    # 文件为空，从20230101开始
                    start_date = "20230101"
                    print(f"文件存在但为空，从 {start_date} 到 {end_date} 获取数据")
            except Exception as e:
                print(f"读取文件出错: {e}，从20230101开始获取数据")
                start_date = "20230101"
        else:
            # 文件不存在，从20230101开始
            start_date = "20230101"
            print(f"文件不存在，从 {start_date} 到 {end_date} 获取数据")
        
        # 检查日期范围是否有效
        if start_date > end_date:
            print(f"起始日期 {start_date} 大于结束日期 {end_date}，跳过")
            continue
        ...
        try:
            ...
            if not stock_zh_a_daily_df.empty:
                ...
                # 检查是否与现有数据有重叠
                if os.path.exists(qfq_file) and os.path.getsize(qfq_file) > 0:
                    try:
                        existing_df = pd.read_csv(qfq_file)
                        if not existing_df.empty:
                            ...
                            data = data[data['date'] > last_date_obj]
                ...
                if not data.empty:
                    # 确定是否需要添加header
                    header = not os.path.exists(qfq_file) or os.path.getsize(qfq_file) == 0
                    
                    # 写入文件
                    data.to_csv(qfq_file, mode="a", index=False, header=header, encoding='utf-8-sig')
                    print(f"成功写入 {len(data)} 条数据到 {qfq_file}")
                else:
                    print("没有新数据需要写入")
            else:
                print(f"没有获取到数据")
