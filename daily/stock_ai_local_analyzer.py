...
# 功能：将股票分析提示词发送给本地部署的Ollama AI，并将分析结果保存为markdown文件
# 实现原理：
# 1. 加载股票历史数据
# 2. 计算支撑位和阻力位
# 3. 获取股票基本信息
...
class StockAILocalAnalyzer:
    ...
    def generate_ai_prompt(self):
        ...
        prompt = f"""你是一位专业的金融分析师，擅长股票技术分析和投资建议。请基于以下股票数据，提供详细的分析和操作建议：
...
"""
    ...
    def save_analysis_to_md(self, analysis_content):
        ...
        # 构建markdown内容
        md_content = f"""# {summary['name']} ({self.ticker}) 股票分析报告
...
"""
    ...
    def run_analysis(self):
        ...
        # 提示用户如何使用
        print("\n=== 分析完成 ===")
        print("分析报告已保存为markdown文件，包含完整的股票分析和操作建议。")
